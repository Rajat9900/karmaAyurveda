...
import {
  bulkImportDiseaseTreatmentsAction,
  BulkDiseaseTreatmentRow,
  BulkTreatmentImportResult
} from '@/app/actions/diseaseTreatmentActions';
import { Disease } from '@/app/actions/diseaseActions';

interface FieldDef {
  key: keyof BulkDiseaseTreatmentRow;
  label: string;
  required?: boolean;
  aliases: string[];
}
...
const FIELDS: FieldDef[] = [
  { key: 'title', label: 'Treatment Title', required: true, aliases: ['title', 'name', 'pagetitle', 'heading'] },
  { key: 'slug', label: 'URL Slug', aliases: ['slug', 'url', 'link', 'urlslug', 'permalink'] },
  { key: 'short_description', label: 'Short Description', aliases: ['shortdescription', 'excerpt', 'summary'] },
  { key: 'content', label: 'Page Content', aliases: ['content', 'pagecontenthtml', 'pagecontent', 'body'] },
...
interface BulkTreatmentUploadClientProps {
  diseases?: Disease[];
}

export default function BulkTreatmentUploadClient({ diseases = [] }: BulkTreatmentUploadClientProps) {
  ...
  const [diseaseId, setDiseaseId] = useState<number | ''>(diseases[0]?.id ?? '');

  const [submitting, setSubmitting] = useState(false);
  const [result, setResult] = useState<BulkTreatmentImportResult | null>(null);
  ...
  const buildMappedRows = (): BulkDiseaseTreatmentRow[] => {
    return rows.map(row => {
      const mapped: BulkDiseaseTreatmentRow = {};
      FIELDS.forEach(field => {
  ...
  const handleImport = async () => {
    if (!diseaseId) {
      setParseError('You must select a parent disease for this batch.');
      return;
    }
    ...
    const aggregate: BulkTreatmentImportResult = { success: true, created: 0, skipped: 0, errors: [] };
    ...
    try {
      for (let start = 0; start < mappedRows.length; start += BATCH_SIZE) {
        ...
        const batchResult = await bulkImportDiseaseTreatmentsAction(batch, start, Number(diseaseId));
  ...
  return (
    <main className="flex-1 overflow-y-auto bg-[#f8fafc] p-8 space-y-6">
      ...
      <div className="text-[10px] font-bold text-slate-400 tracking-wider flex items-center gap-2 uppercase">
        ...
        <Link href="/admin/treatment-pages" className="hover:text-slate-650">Treatment Pages</Link>
        <span className="text-slate-300">/</span>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black text-slate-900 tracking-tight">Bulk Upload Treatment Pages</h2>
            <p className="text-xs font-semibold text-slate-400 mt-0.5">Import multiple treatment sub-pages at once from a CSV or Excel file</p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.01)] p-6 space-y-3">
        <div className="flex items-center gap-1.5 text-[10px] font-black text-slate-400 uppercase tracking-widest">
          ...
          Step 1 — Choose Parent Disease*
        </div>
        <p className="text-[10px] font-medium text-slate-400 -mt-1">
          Every treatment page in this file belongs to exactly one disease — pick the disease whose "Treatments We Offer" cards this whole batch should appear under.
        </p>
        {diseases.length === 0 ? (
          ...
          <select
            ...
            className="w-full sm:w-80 px-3 py-2.5 rounded-lg border border-slate-200 focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 outline-none text-xs bg-white font-semibold text-slate-800"
          >
            {diseases.map(d => (
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.01)] p-6 space-y-4">
        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Step 2 — Choose File</span>
      ...
      {headers.length > 0 && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.01)] p-6 space-y-4">
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Step 3 — Link Excel Columns to Treatment Fields</span>
      ...
      {previewRows.length > 0 && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.01)] p-6 space-y-4">
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Step 4 — Preview (first {previewRows.length} rows)</span>
          ...
          <div className="pt-4 border-t border-slate-100 flex justify-end">
            <button
              ...
              disabled={submitting || !diseaseId}
              className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs rounded-xl flex items-center gap-2 shadow-sm cursor-pointer disabled:opacity-60 transition-all"
            >
              {submitting ? (
                <>
                ...
                <>
                  Import {rows.length} Treatment Page{rows.length === 1 ? '' : 's'}
